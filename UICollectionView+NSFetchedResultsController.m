...
- (void)addChangeForObjectAtIndexPath:(NSIndexPath *)indexPath forChangeType:(NSFetchedResultsChangeType)type newIndexPath:(NSIndexPath *)newIndexPath {
	...
	/// It seems ios 8 send both "change" and "move" event for a new/deleteSection event or for a simple object move
	/// However ios 9 and up sometime will send move event and sometime a simple change event. It's awful to handle

	BOOL objectRelatedToSectionUpdate_ = [self.insertedSectionIndexes containsIndex:newIndexPath.section] || [self.deletedSectionIndexes containsIndex:indexPath.section];
	if  (([[[UIDevice currentDevice] systemVersion] compare:@"9.0" options:NSNumericSearch] != NSOrderedAscending) &&
			 (type == NSFetchedResultsChangeUpdate && (objectRelatedToSectionUpdate_ || (newIndexPath != nil && newIndexPath != indexPath))))
	{
			type = NSFetchedResultsChangeMove;
			newIndexPath = newIndexPath ?: indexPath;
	}
	...
	switch(type)
	{
		...
		case NSFetchedResultsChangeInsert:
		{
      if (indexPath == newIndexPath) break ;
      ...
      if (indexPath != newIndexPath) break ;
          ...
          ([[[UIDevice currentDevice] systemVersion] compare:@"9.0.0" options:NSNumericSearch] == NSOrderedAscending))
				return ;
...
- (void)commitChanges
{
  ...
  NSInteger totalChanges = ([self.deletedSectionIndexes count] + [self.insertedSectionIndexes count]) +
                           ([self.deletedItemIndexPaths count] + [self.insertedItemIndexPaths count] + [self.updatedItemIndexPaths count]);

	if (self.window == nil || totalChanges > 50)
  {
		[self clearChanges];
		[self reloadData];
